...
async function summarizeAndroidDevice(validations) {
  const logFile = "logs/android-device-discovery.log";
  const logPath = path.join(runDir, logFile);
  const result = await run("node", ["scripts/dpm/discover_android_device.mjs", path.relative(repoRoot, runDir)], {
    cwd: repoRoot,
    logPath,
    display: "node scripts/dpm/discover_android_device.mjs <runDir>",
  });
  validations.push({
    command: "node scripts/dpm/discover_android_device.mjs <runDir>",
    cwd: ".",
    status: result.exitCode === 0 ? "passed" : "failed",
    exitCode: result.exitCode,
    log: logFile,
...
async function readAndroidDeviceDiscovery() {
  try {
    const raw = await fs.readFile(path.join(runDir, "json", "android_device_discovery.json"), "utf8");
    return JSON.parse(raw);
  } catch {
    return null;
  }
...
function androidDeviceMarkdown(discovery) {
  if (!discovery) return "- Android device discovery was not generated.";
  return [
    `- Status: ${discovery.status}`,
    `- Device type: ${discovery.deviceType}`,
    `- adb available: ${discovery.adbAvailable ? "yes" : "no"}`,
    `- Devices detected: ${discovery.devices?.length || 0}`,
    `- Owner action: ${discovery.ownerAction}`,
  ].join("\n");
}
...
async function main() {
  ...
  await summarizeAndroidDevice(validations);
  ...
  const androidDevice = await readAndroidDeviceDiscovery();
  const webCiVerdict = blocked ? "BLOCKED" : failed.length > 0 ? "WARNING" : isGitHubWorkflowRun ? "PASS" : "WARNING";
  const realDeviceVerdict = androidDevice?.status === "DEVICE_CONNECTED"
    ? "READY_FOR_DEVICE_QA"
    : androidDevice?.status === "EMULATOR_CONNECTED"
      ? "WARNING_EMULATOR_ONLY"
      : "WARNING_DEVICE_REQUIRED";
  const fieldPilotVerdict = realDeviceVerdict === "READY_FOR_DEVICE_QA" && webCiVerdict === "PASS"
    ? "CONDITIONAL_DEVICE_TEST_REQUIRED"
    : "NOT_READY_REAL_DEVICE_PENDING";
