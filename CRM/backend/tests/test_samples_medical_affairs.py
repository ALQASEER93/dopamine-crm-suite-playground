...
def test_medical_affairs_endpoints_and_reports(
    ...
    roi_report = client.get("/api/v1/medical-affairs/reports/event-roi", headers=manager_headers)
    assert roi_report.status_code == 200, roi_report.text
    assert any(row["event_id"] == event_id for row in roi_report.json())
