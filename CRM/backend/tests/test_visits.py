...
def test_visit_lifecycle_enforces_status(client, auth_headers):
    doctors = client.get("/api/v1/doctors", headers=auth_headers).json()["data"]
    doctor_id = doctors[0]["id"] if doctors else None
    if not doctor_id:
        doctor_resp = client.post(
            "/api/v1/doctors",
            headers=auth_headers,
            json={"name": "Dr. Lifecycle", "specialty": "Internal", "area": "Central"},
        )
        doctor_id = doctor_resp.json()["id"]
    ...
    assert start_again.status_code == 400, start_again.text
