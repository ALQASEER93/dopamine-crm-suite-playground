...
@router.get(
    "/reports/event-roi",
    response_model=list[MedicalEventROIOut],
    dependencies=[Depends(require_roles("admin", "sales_manager"))],
)
def report_event_roi(
    from_date: str | None = None,
    to_date: str | None = None,
    db: Session = Depends(get_db),
) -> list[MedicalEventROIOut]:
    from_dt = _parse_iso_datetime(from_date)
    to_dt = _parse_iso_datetime(to_date)
    ...
    rows: list[MedicalEventROIOut] = []
    for event in events:
        budget = float(event.budget or Decimal("0"))
        actual_cost = float(event.actual_cost or Decimal("0"))
        revenue_impact = float(event.revenue_impact or Decimal("0"))
        roi_value = revenue_impact - actual_cost
        roi_percent = None
        if actual_cost > 0:
            roi_percent = round((roi_value / actual_cost) * 100.0, 2)
        rows.append(
            MedicalEventROIOut(
                event_id=event.id,
                title=event.title,
