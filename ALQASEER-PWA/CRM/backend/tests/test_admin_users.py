...
def test_admin_users_role_change_clears_rep_profile(client: TestClient, auth_headers: dict[str, str]) -> None:
    territories = client.get("/api/v1/territories?page=1&pageSize=10", headers=auth_headers).json()
    territory_id = None
    if territories.get("data"):
        territory_id = territories["data"][0]["id"]
    ...
    payload = {
        "name": "Role Flip Rep",
        "email": "role.flip.rep@example.com",
        "password": "Rep12345!",
        "userType": "medical_rep",
        "territoryId": territory_id,
    }
    ...
    assert created.status_code == 201, created.text
    user_id = created.json()["id"]
    assert created.json()["role"]["slug"] == "medical_rep"
    assert created.json()["salesRep"] is not None

    update_payload = {"userType": "admin"}
    updated = client.patch(f"/api/v1/admin/users/{user_id}", json=update_payload, headers=auth_headers)
    assert updated.status_code == 200, updated.text
    assert updated.json()["role"]["slug"] == "admin"
    assert updated.json()["salesRep"] is None
