...
def _is_missing_preview_schema(exc: ProgrammingError, *expected_terms: str) -> bool:
    message = str(getattr(exc, "orig", exc)).lower()
    return any(term.lower() in message for term in expected_terms) and (
...
def list_customers(
    ...
    try:
        ...
        if not _is_missing_preview_schema(exc, "route_accounts", "routes"):
            raise
        db.rollback()
        route_accounts = []
    ...
    if normalized_type in {"", "doctor"}:
        ...
        try:
            ...
            if not _is_missing_preview_schema(exc, "doctors"):
                raise
            db.rollback()
            doctors = []
    ...
    if normalized_type in {"", "pharmacy"}:
        ...
        try:
            ...
            if not _is_missing_preview_schema(exc, "pharmacies"):
                raise
            db.rollback()
            pharmacies = []
...
def list_visits(
    ...
    try:
        ...
        if not _is_missing_preview_schema(exc, "visits", "visit_date"):
            raise
        db.rollback()
        return []
    results = []
